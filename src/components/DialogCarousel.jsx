...
const DialogCarousel = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        ref={dialogRef}
        className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full rounded-2xl shadow-2xl overflow-hidden transform perspective-1000 ${className}`}
      ...
      >
            ...
            {showArrows && normalizedSlides.length > 1 && (
              <div className="flex items-center gap-1 mr-2">
                ...
                <button
                  onClick={goToNext}
                  disabled={
                    currentIndex === normalizedSlides.length - 1 && !loop
                  }
                  className="p-1.5 rounded-lg bg-surface disabled:hover:bg-surface not-disabled:hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                >
        ...
        <div
          ...
          className="relative overflow-hidden min-h-30"
        >
                ...
                >
                  ...
                  {slide.showIcon && (
                    <div className="flex justify-center mb-4">
                      <div
                        className={`p-4 rounded-full ${slideConfig.bgLight} animate-pulse`}
                      >
                        <SlideIcon
                          className={`w-8 h-8 ${slideConfig.textColor}`}
                        />
                      </div>
                    </div>
                  )}
                    ...
                    >
                      ...
                      <div className="relative overflow-hidden aspect-video">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center gap-4 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300">
                          <button
                            onClick={() => handleClickPopup(slide.link)}
                            className="p-3 rounded-full glass hover:bg-primary text-primary-foreground hover:text-primary-foreground transition-all"
                          >
                            <ArrowUpRight className="w-5 h-5" />
                          </button>
                          <button
                            onClick={() => handleClickPopup(slide.github)}
                            className="p-3 rounded-full glass hover:bg-primary text-primary-foreground hover:text-primary-foreground transition-all"
                          >
        ...
        <div
          ref={buttonsRef}
          className="flex justify-center items-center gap-3 p-6 pt-2 glass-strong"
        >
          {/* Dots Indicator */}
